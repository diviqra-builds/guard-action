...
async function scanText(
  ...
  baseUrl: string,
  context: string,
): Promise<{ blocked: boolean; score: number; threat: string | null; reason: string | null; wall: number | null; latency_ms: number }> {
...
function extractTexts(filePath: string): string[] {
      ...
      function extract(obj: unknown): void {
        ...
        if (obj && typeof obj === 'object') {
          for (const key of ['text', 'content', 'prompt', 'message', 'input', 'query']) {
            if ((obj as Record<string, unknown>)[key]) extract((obj as Record<string, unknown>)[key])
          }
        }
      }
      extract(parsed)
  ...
  // .txt or other — return whole file
